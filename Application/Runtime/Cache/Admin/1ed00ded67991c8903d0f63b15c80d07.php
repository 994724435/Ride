...
<?php if(is_array($n_nav)): foreach($n_nav as $key=>$val): ?><div>
    ...
    <div class="article_list">
      ...
      <form action="" method="post">
        <div class="seo_table1">
          <p>网站关键字:</p>
          <input type="text" value="<?php echo ($seo["seo_keyword"]); ?>" name="keyword">
        </div>
        <div class="seo_table2">
          <p>&nbsp;网站描述:</p>
          <textarea name="describe" id="" cols="30" rows="10"><?php echo ($seo["seo_describe"]); ?></textarea>
        </div>
  ...
  if(adm_power.indexOf('基础设置') === -1){
    $('.seo_submit').addClass('friendShip1_notice_block show_block').attr('type','button');
  }
  </script>
  <!--修改权限弹框-->
