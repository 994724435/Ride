...
global.jumpdelay = window.debug ? 20000000 : 2000;
...
// -----------------------------------> shop 相关业务逻辑代码 start
/*
global.user_read = function(uid) {
	var user = null;
	if(!uid) {
		// 尝试从 pdata 中获取
		var s = $.pdata('user');
		if(!s) {
			user = require('/user-read.htm');
		} else {
			user = json_decode(s);
			if(!user) {
				user = require('/user-read.htm');
		...
		// 可能没有权限
		user = require('/user-read-'+uid+'.htm');
	}
	return user;
}

global.user_update = function(uid, user) {
	// 可能没有权限
	var user2 = user_read(uid);
	user2 = $.extend(user2, user);
	$.pdata('user', user2);
...
function eval_script(arr, key, args) {
	if(!arr) return;
...
function get_title_body_script_css(s) {
	var s = $.trim(s);
