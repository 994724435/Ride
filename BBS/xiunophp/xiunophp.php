...
/*
	XiunoPHP 3.0，采用简单结构，有利于 HHVM 编译 / opcode 缓存，提前与 PHP7 契合做好准备
	1. 不要 include 变量
...
$db->errno AND message(-1, $db->errstr); // 安装的时候检测过了，不必每次都检测。但是要考虑环境移植。
...
function array_assoc_slice($arrlist, $start, $length = 0) {
	if(isset($arrlist[0])) return array_slice($arrlist, $start, $pagesize);
...
function db_new($dbconf) {
	...
	if($dbconf) {
		switch ($dbconf['type']) {
			...
			default: message(-1, '不支持的 db type:'.$dbconf['type']);
		}
		if(!$db || ($db && $db->errstr)) message(-1, $db->errstr);
		return $db;
	}
	return NULL;
}
...
function cache_new($cacheconf) {
	...
	if($cacheconf && $cacheconf['enable']) {
		switch ($cacheconf['type']) {
			...
			default: message(-1, '不支持的 cache type:'.$conf['cache']['type']);
		}
		if(!$cache || ($cache && $cache->errstr)) message(-1, $cache->errstr);
		return $cache;
	}
	return NULL;
}
