...
class cache_redis {
        ...
        public function connect() {
                ...
                $redis = new Redis;
                $r = $redis->connect($conf['host'], $conf['port']);
                if(!$r) {
        ...
        public function set($k, $v, $life = 0) {
                ...
                $r = $this->link->hmset($k, $v);
                $life AND $r AND $this->link->expire($k, $life);
                return $r;
        }
        public function get($k) {
                if(!$this->link && !$this->connect()) return FALSE;
                $r = $this->link->hgetall($k);
                return $r === FALSE ? NULL : $r;
        }
        public function delete($k) {
                if(!$this->link && !$this->connect()) return FALSE;
                return $this->link->hdel($k);
        }
        public function truncate() {
                if(!$this->link && !$this->connect()) return FALSE;
...
class cache_mysql {
        ...
        public function get($k) {
                if(!$this->link && !$this->connect()) return FALSE;
                $time = time();
                $arr = $this->db->find_one("SELECT * FROM `bbs_cache` WHERE k='$k'");
                if(!$arr) return NULL;
                if($arr['expiry'] && $time > $arr['expiry']) {
                        $this->db->exec("DELETE FROM `bbs_cache` WHERE k='$k'", $this->link);
                        return NULL;
                }
        ...
        public function delete($k) {
                if(!$this->link && !$this->connect()) return FALSE;
                $r = $this->db->exec("DELETE FROM `bbs_cache` WHERE k='$k'", $this->link);
                return empty($r) ? FALSE : TRUE;
        }
        public function truncate() {
                if(!$this->link && !$this->connect()) return FALSE;
                $r = $this->db->exec("TRUNCATE `bbs_cache`", $this->link);
                return TRUE;
        }
        public function error($errno = 0, $errstr = '') {
