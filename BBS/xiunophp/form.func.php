...
function form_checkbox_yes_no($name, $checked = 0) {
	$checked = intval($checked);
	return form_checkbox($name,  array(1=>'', 0=>''), $checked);
}
...
function form_checkbox($name, $arr, $ischecked) {
	$s = '';
	$add = $ischecked ? ' checked="checked"' : '';
	$checkedtext = $arr[$ischecked];
	$s .= "<label><input type=\"checkbox\" name=\"$name\" value=\"1\" class=\"noborder\"$add />$checkedtext</label> \r\n";
	return $s;
}
